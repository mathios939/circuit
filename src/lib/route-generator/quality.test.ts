...
describe("assessRouteQuality", () => {
  it("accepts a clean loop of the right length", () => {
    ...
    const report = assessRouteQuality({ coordinates: coords, distanceM: 20_000, targetM: 20_000, start, waypointCount: 4, isLoop: true, profile: road });
  ...
  it("rejects out-and-back geometries", () => {
    ...
    const report = assessRouteQuality({ coordinates: coords, distanceM: 20_000, targetM: 20_000, start, waypointCount: 3, isLoop: true, profile: road });
  ...
  it("rejects loops outside the distance tolerance", () => {
    const report = assessRouteQuality({ coordinates: circleLoop(20), distanceM: 20_000, targetM: 25_000, start, waypointCount: 4, isLoop: true, profile: road, maxDistanceError: 0.1 });
  ...
  it("flags broken geometries (jumps, too short, inconsistent length)", () => {
    ...
    expect(assessRouteQuality({ coordinates: jump, distanceM: 10_000, start, waypointCount: 4, isLoop: true, profile: road }).geometryValid).toBe(false);
    expect(assessRouteQuality({ coordinates: [start, start], distanceM: 50, start, waypointCount: 2, isLoop: true, profile: road }).geometryValid).toBe(false);
    const inconsistent = assessRouteQuality({ coordinates: circleLoop(10), distanceM: 40_000, start, waypointCount: 4, isLoop: true, profile: road });
  ...
  it("penalises ways unsuited to the activity", () => {
    ...
    const forRoadBike = assessRouteQuality({ coordinates: coords, distanceM: 20_000, targetM: 20_000, start, waypointCount: 4, isLoop: true, profile: road, segments });
    const forMtb = assessRouteQuality({ coordinates: coords, distanceM: 20_000, targetM: 20_000, start, waypointCount: 4, isLoop: true, profile: getActivityProfile("mtb"), segments });
